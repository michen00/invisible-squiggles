...
- **Tooling suites** (`scripts/test-*.sh`, `scripts/test-*.cjs`, run by `make test`): cover repository tooling -- release guards, the PR-title checker, the CI gate contract -- not the extension. They need no VSCode host and live beside what they test, deliberately outside `src/test/`.
