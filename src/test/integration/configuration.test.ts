...
suite('Extension Integration Tests - Configuration', () => {
  ...
  test('workbench.colorCustomizations should update when toggle command executes', async () => {
    ...
    assert.ok(
      ...
      `Color customizations should be updated. Config changed: ${changed}, Before: ${beforeJson.substring(
        0,
        100
      )}, After: ${afterJson.substring(0, 100)}`
    );
  });
  ...
  test('All 16 configuration scenarios should work correctly', async () => {
    ...
    assert.strictEqual(
      allConfigs.length,
      16,
      'Should have 16 configuration combinations'
    );
